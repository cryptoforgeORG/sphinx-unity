...
#pragma warning restore 0414, 0067, 0649
        ...
        public void LogToConsole(string logText, bool newline = true)
        {
            ...
            if (logExists)
            {
                LogToConsole(new Log(logText), newline);
            }
        }
        ...
        /// <param name="log">Log to be logged.</param>
        /// <param name="newline">If a newline should be ins</param>
        public void LogToConsole(ILog log, bool newline = true)
        {
            FlushQueuedLogs();
            AppendLog(log, newline);
            OnLog?.Invoke(log);
        ...
        protected void AppendLog(ILog log, bool newline = true)
        {
            _logStorage.AddLog(log, newline);
            RequireFlush();
        }
        ...
        {
            if (_logStorage != null) _logStorage.MaxStoredLogs = _maxStoredLogs;
            if (_logQueue != null) _logQueue.MaxStoredLogs = _maxStoredLogs;
        }
    }
}
